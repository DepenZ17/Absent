                    ...
                    <div class="container-fluid">
                        ...
                        <h3>Data Kehadiran</h3>
                            <form class="form-header" action="prospenab.php" method="POST">
                                <input autocomplete="off" class="au-input au-input--xl" type="text" name="cari" placeholder="Cari NIP atau Nama Karyawan" />
        ...
        <div class="main-content">
            ...
            <table class="table table-borderless table-striped table-earning">
                ...
                <tbody>
                    ...
                    // Paginasi
                    $batas = 5; // Batas jumlah data per halaman
                    $halaman = isset($_GET['halaman']) ? (int)$_GET['halaman'] : 1; // Halaman saat ini
                    $halaman_awal = ($halaman > 1) ? ($halaman * $batas) - $batas : 0; // Posisi awal data

                    // Mengambil total data
                    $data = mysqli_query($koneksi, "SELECT * FROM tb_kehadiran");
                    $jumlah_data = mysqli_num_rows($data); // Total data
                    $total_halaman = ceil($jumlah_data / $batas); // Total halaman

                    // Mengambil data dengan limit
                    $query = mysqli_query($koneksi, "SELECT * FROM tb_kehadiran LIMIT $halaman_awal, $batas");
                    $no = $halaman_awal + 1; // Nomor urut
                    ...
                    while ($row = mysqli_fetch_array($query)) {
                        ?>
                            <tr>
                                ...
                                <td>
                                    <a href="absen/hapus_absen.php?id=<?php echo $row['id_absensi']; ?>">
